...
namespace Scada.Scheme.Editor
{
	...
	public partial class FrmMain : Form, IMainForm
	{
		...
		private void SetComponentImages()
		{
			...
			//check if items index 2 (dynamic text) in the listview and add them to the image list
			if (lvCompTypes.Items.Count > 1)
			{
			...
			//check if items 4 exists (dynamic picture) in the listview and add them to the image list
			if (lvCompTypes.Items.Count > 4)
			{
		...
		private void FillComponentTypes()
		{
			try
			{
				...
				bool containsShapeCompLibSpec = specs.Any(spec => spec.ToString().Contains("Scada.Web.Plugins.SchShapeComp.ShapeCompLibSpec"));

				//Remove lvCompTypes.Items index 2 (dynamic text) and 4 (dynamic picture) if compManager contains PlgSchShapeComp.dll component
				if (containsShapeCompLibSpec)
				{
					this.lvCompTypes.Items.Remove(lvCompTypes.Items[2]);
					this.lvCompTypes.Items.Remove(lvCompTypes.Items[3]);
				}
